...
Es freuen sich auf Euch mehr als 14 Sch&auml;fchen: Paula, Jutta, Afra, Lila, ... B&ouml;ckl.
    ...
    <div class="content moretext bodyMiddleOnly">
        ...
        foreach ($horses as $horseline) {
            //skip this entry?
            if ($horseline == $_SERVER['PHP_SELF'] #['SCRIPT_NAME']
			...
			/*if (count($parts) < 1)
				# Skip highest level directories without any structuring in the filename?
				continue;
			*/
            ...
            foreach ($horse_images as $file) {
                ...
                {
                    // Generate the small size preview (thumbnail):
                    ...
                    $output = `convert $imagelink -resize x90 $previewlink`;
                    #echo $output;
                }
            ?>
                <span class="curl">
